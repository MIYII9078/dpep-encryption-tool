package crypto

import (
	"crypto/sha256"
	"io"

	"golang.org/x/crypto/hkdf"
)

const AESCipherRounds = 10

var aesCipherSbox = [256]byte{
	,
}

var aesCipherInvSbox = [256]byte{
	,
}

func AESCipherEncrypt(block, key []byte) []byte {
	if len(block) != 16 || len(key) < 16 {
		panic("invalid AESCipher input")
	}
	state := make([]byte, 16)
	copy(state, block)
	roundKeys := deriveAESCipherKeys(key)
	xorBytes(state, roundKeys[0])
	for r := 1; r <= AESCipherRounds; r++ {
		subBytes(state)
		shiftRows(state)
		if r != AESCipherRounds {
			mixColumns(state)
		}
		xorBytes(state, roundKeys[r])
	}
	return state
}

func AESCipherDecrypt(block, key []byte) []byte {
	if len(block) != 16 || len(key) < 16 {
		panic("invalid AESCipher input")
	}
	state := make([]byte, 16)
	copy(state, block)
	roundKeys := deriveAESCipherKeys(key)
	for r := AESCipherRounds; r >= 1; r-- {
		xorBytes(state, roundKeys[r])
		if r != AESCipherRounds {
			invMixColumns(state)
		}
		invShiftRows(state)
		invSubBytes(state)
	}
	xorBytes(state, roundKeys[0])
	return state
}

func deriveAESCipherKeys(key []byte) [][]byte {
	master := key[:16]
	keys := make([][]byte, AESCipherRounds+1)
	reader := hkdf.Expand(sha256.New, master, []byte("aescipher-v2"))
	for i := 0; i <= AESCipherRounds; i++ {
		k := make([]byte, 16)
		_, err := io.ReadFull(reader, k)
		if err != nil {
			panic(err)
		}
		keys[i] = k
	}
	return keys
}

func shiftRows(state []byte) {
	state[1], state[5], state[9], state[13] = state[5], state[9], state[13], state[1]
	state[2], state[6], state[10], state[14] = state[10], state[14], state[2], state[6]
	state[3], state[7], state[11], state[15] = state[15], state[3], state[7], state[11]
}

func invShiftRows(state []byte) {
	state[1], state[5], state[9], state[13] = state[13], state[1], state[5], state[9]
	state[2], state[6], state[10], state[14] = state[10], state[14], state[2], state[6]
	state[3], state[7], state[11], state[15] = state[7], state[11], state[15], state[3]
}

func mixColumns(state []byte) {
	for i := 0; i < 4; i++ {
		col := i * 4
		a0, a1, a2, a3 := state[col], state[col+1], state[col+2], state[col+3]
		state[col] = gfMul(2, a0) ^ gfMul(3, a1) ^ a2 ^ a3
		state[col+1] = a0 ^ gfMul(2, a1) ^ gfMul(3, a2) ^ a3
		state[col+2] = a0 ^ a1 ^ gfMul(2, a2) ^ gfMul(3, a3)
		state[col+3] = gfMul(3, a0) ^ a1 ^ a2 ^ gfMul(2, a3)
	}
}

func invMixColumns(state []byte) {
	for i := 0; i < 4; i++ {
		col := i * 4
		a0, a1, a2, a3 := state[col], state[col+1], state[col+2], state[col+3]
		state[col] = gfMul(0x0e, a0) ^ gfMul(0x0b, a1) ^ gfMul(0x0d, a2) ^ gfMul(0x09, a3)
		state[col+1] = gfMul(0x09, a0) ^ gfMul(0x0e, a1) ^ gfMul(0x0b, a2) ^ gfMul(0x0d, a3)
		state[col+2] = gfMul(0x0d, a0) ^ gfMul(0x09, a1) ^ gfMul(0x0e, a2) ^ gfMul(0x0b, a3)
		state[col+3] = gfMul(0x0b, a0) ^ gfMul(0x0d, a1) ^ gfMul(0x09, a2) ^ gfMul(0x0e, a3)
	}
}

func gfMul(a, b byte) byte {
	var p byte = 0
	for i := 0; i < 8; i++ {
		if b&1 != 0 {
			p ^= a
		}
		hi := a & 0x80
		a <<= 1
		if hi != 0 {
			a ^= 0x1b
		}
		b >>= 1
	}
	return p
}

func subBytes(state []byte) {
	for i := range state {
		state[i] = aesCipherSbox[state[i]]
	}
}

func invSubBytes(state []byte) {
	for i := range state {
		state[i] = aesCipherInvSbox[state[i]]
	}
}

func xorBytes(a, b []byte) {
	for i := range a {
		a[i] ^= b[i]
	}
}
